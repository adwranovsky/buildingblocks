...
def tx(clk, reset, serial_out, byte_in, start, done, baud):
    ...
    @always_comb
    def fsm_comb_logic():
        done.next = state==t_state.BUSY and bit_count == bit_count.max-1
        full_bit_tstart.next = (state==t_state.IDLE and start) or (state==t_state.BUSY and full_bit_done and not done)
    ...
    def fsm_seq_logic():
        ...
        if state == t_state.IDLE:
            ...
            if full_bit_done:
                shift_reg.next = concat(1, shift_reg[9:])
            if done:
